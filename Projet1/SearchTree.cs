...
namespace ProjetIA
{
    class SearchTree
    {
        ...
        public List<GenericNode> RechercheSolutionAEtoile(GenericNode N0, ListBox ouverts, ListBox fermes)
        {
            ...
            fermes.Items.Add("{ " + L_Fermes.Aggregate("", (list, x) => list + (list == "" ? "" : ", ") + ((Node2)x).ToString()) + " }");
            ouverts.Items.Add("{ " + L_Ouverts.Aggregate("", (list, x) => list + (list == "" ? "" : ", ") + ((Node2)x).ToString()) + " }");

            // tant que le noeud n'est pas terminal et que ouverts n'est pas vide
            while (L_Ouverts.Count != 0)
            {
